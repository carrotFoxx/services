...
from imagesmanager.adapters import ImageMongoStorageAdapter, ImageArchiveMongoStorageAdapter
from imagesmanager.api import ImageManagerAPI
...
class ImageManagerApp(RPCServerApplication, CommonAppMixin):
    async def _setup(self):
        await super()._setup()

        repository = Repository(ImageMongoStorageAdapter())
        archive = Repository(ImageArchiveMongoStorageAdapter())
...
if __name__ == '__main__':
    ROOT_LOG.info('starting [%s]', ImageManagerApp.__name__)
    ImageManagerApp().run()
